...
export default function Settings() {
  ...
  return (
    <section className="mx-auto w-full max-w-5xl space-y-6">
      <div className="rounded-[2rem] border border-white/10 bg-slate-900/70 p-5 backdrop-blur sm:p-8">
      ...
      <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="mt-6 grid grid-cols-1 gap-3 sm:max-w-xs sm:grid-cols-2">
            {SETTINGS_OPTIONS.delays.map((delay) => (
              <button
                ...
                className={[
                  "min-h-12 rounded-2xl px-5 py-4 text-sm font-medium transition",
        ...
        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 sm:p-7">
          ...
          {[
            ...
            <label
              key={key}
              className="flex items-start justify-between gap-4 rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-4 sm:items-center"
            >
              <div>
              ...
              <input
                ...
                className="mt-1 h-5 w-5 shrink-0 accent-rose-500 sm:mt-0"
              />
            </label>
          ))}
          </div>
